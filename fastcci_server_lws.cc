...
// Simple tracing helper (portable across LWS versions)
static inline void trace(const char *where, int reason, int extra = 0) {
  lwsl_notice("%s: reason=%d extra=%d\n", where, reason, extra);
}
...
struct resultList {
  ...
  resultList(int initialSize = 1024 * 1024)
      : max(initialSize), num(0), tags(NULL) {
...
// ---------- minimal logging + uri helper, portable across LWS versions
// ----------
static inline const char *safe_proto_name(struct lws *wsi) {
  const struct lws_protocols *p = lws_get_protocol(wsi);
  return p && p->name ? p->name : "(null)";
}
static inline void get_uri(struct lws *wsi, char *dst, size_t dstlen) {
  if (!dst || !dstlen)
    return;
#if defined(LWS_WITH_HTTP_UNCOMMON_HEADERS) || 1
  if (lws_hdr_copy(wsi, dst, (int)dstlen, WSI_TOKEN_GET_URI) <= 0)
    dst[0] = 0;
#else
  dst[0] = 0;
#endif
}
#define TRACE_NOTICE(fmt, ...) lwsl_notice(fmt "\n", ##__VA_ARGS__)
...
static int64_t to_i64(const char *s) {
  if (!s)
    return -1;
  if (*s == '\0')
    return -1; // treat empty as not provided
...
static Query parse_query(struct lws *wsi) {
  ...
  auto a = get_arg(wsi, "a");
  if (!a.empty())
    q.a = a;
  auto o = get_arg(wsi, "o");
  if (!o.empty())
    q.o = atoi(o.c_str());
  auto s = get_arg(wsi, "s");
  if (!s.empty())
    q.s = atoi(s.c_str());
  return q;
}
...
static void emit_line(const std::function<void(const std::string &)> &emit,
                      const std::string &s) {
  emit(s + "\n");
}
static void result_start(const std::function<void(const std::string &)> &emit) {
  emit_line(emit, "COMPUTE_START");
}
static void result_done(const std::function<void(const std::string &)> &emit) {
  emit_line(emit, "DONE");
}
static void result_printf(const std::function<void(const std::string &)> &emit,
                          const char *fmt, ...) {
  char buf[512];
  va_list ap;
  va_start(ap, fmt);
...
static void result_flush(const std::function<void(const std::string &)> &emit,
                         Accum &acc) {
  if (!acc.buf.empty()) {
    emit_line(emit,
              std::string("RESULT ") + acc.buf.substr(0, acc.buf.size() - 1));
    acc.buf.clear();
    acc.count = 0;
  }
}
static void result_queue(const std::function<void(const std::string &)> &emit,
                         Accum &acc, result_type item, unsigned char tag) {
  char tmp[64];
  snprintf(tmp, sizeof(tmp), "%d,%d,%d|", int(item & cat_mask),
           int((item & depth_mask) >> depth_shift), tag);
  acc.buf.append(tmp);
  if (++acc.count == Accum::maxqueue)
...
static void tagCat_emit(tree_type sid, tree_type did, int maxDepth,
                        resultList *r1,
  ...
  if (foundPath) {
    ...
    int len = i;
    while (i--)
      result_queue(emit, acc,
                   history[i] + (result_type(len - i) << depth_shift), 0);
    result_flush(emit, acc);
  } else {
...
static void
traverse_emit(int outstart, int outsize, resultList *r1,
              const std::function<void(const std::string &)> &emit) {
  Accum acc;
  int outend = outstart + outsize;
  if (outend > r1->num)
    outend = r1->num;
  for (int i = outstart; i < outend; ++i) {
    result_type r = r1->buf[i] & cat_mask;
    result_queue(emit, acc, r1->buf[i],
                 r1->tags == NULL ? goodImages->tags[r] : r1->tags[r]);
  }
  result_flush(emit, acc);
  result_printf(emit, "OUTOF %d", r1->num);
}

static void notin_emit(int outstart, int outsize, resultList *r1,
                       resultList *r2,
                       const std::function<void(const std::string &)> &emit) {
  Accum acc;
  int n = 0, i;
  ...
  if (i == r1->num)
    ...
    result_printf(emit, "OUTOF %d", (outend * r1->num) / i);
}

static void
intersect_emit(int outstart, int outsize, resultList *r1, resultList *r2,
               const std::function<void(const std::string &)> &emit) {
  Accum acc;
  int n = 0;
  int outend = outstart + outsize;
  if (r2->num == 0) {
    result_printf(emit, "OUTOF %d", 0);
    return;
  }
  for (int i = 0; i < r1->num; ++i) {
    ...
    if (r >= maxcat)
      continue;
    result_type m = r2->mask[r];
    if (m != 0) {
      n++;
      if (n <= outstart)
        continue;
      result_queue(emit, acc, r1->buf[i] + ((m - 1) << depth_shift),
...
static void findFQV_emit(int outstart, int outsize, resultList *r1,
  ...
  if (k == 5)
    ...
    result_printf(emit, "OUTOF %d",
                  (outend * r1->num * 3) / ((k - 1) * r1->num + i));
}
...
static void destroy_msg(void *p) {
  auto *m = (msg *)p;
  if (m && m->data)
    free(m->data);
}
...
static void job_result_printf(WorkItem *wi, const char *fmt, ...) {
  char buf[512];
  va_list ap;
  va_start(ap, fmt);
  vsnprintf(buf, sizeof(buf), fmt, ap);
  va_end(ap);
  emit_item(wi, std::string(buf) + "\n");
}
static void job_result_flush(WorkItem *wi, Accum &acc) {
  if (!acc.buf.empty()) {
    emit_item(wi, std::string("RESULT ") +
                      acc.buf.substr(0, acc.buf.size() - 1) + "\n");
    acc.buf.clear();
    acc.count = 0;
  }
}
static void job_result_queue(WorkItem *wi, Accum &acc, result_type item,
                             unsigned char tag) {
  char tmp[64];
  snprintf(tmp, sizeof(tmp), "%d,%d,%d|", int(item & cat_mask),
           int((item & depth_mask) >> depth_shift), tag);
  acc.buf.append(tmp);
  if (++acc.count == Accum::maxqueue)
    job_result_flush(wi, acc);
}

static void tagCat_emit_item(tree_type sid, tree_type did, int maxDepth,
                             resultList *r1, WorkItem *wi) {
  rbClear(rb);
  bool c2isFile = (cat[did] < 0);
  int depth = -1;
  result_type id = sid;
...
static void traverse_emit_item(WorkItem *wi, int outstart, int outsize,
                               resultList *r1) {
  Accum acc;
  int outend = outstart + outsize;
  if (outend > r1->num)
  ...
  for (int i = outstart; i < outend; ++i) {
    if (wi && wi->cancel.load())
      break;
    result_type r = r1->buf[i] & cat_mask;
    job_result_queue(wi, acc, r1->buf[i],
                     r1->tags == NULL ? goodImages->tags[r] : r1->tags[r]);
  }
  job_result_flush(wi, acc);
  job_result_printf(wi, "OUTOF %lld", (long long)r1->num);
}
static void notin_emit_item(WorkItem *wi, int outstart, int outsize,
                            resultList *r1, resultList *r2) {
  Accum acc;
  long long n = 0;
  int i;
  int outend = outstart + outsize;
  ...
  job_result_flush(wi, acc);
  if (i == r1->num)
    job_result_printf(wi, "OUTOF %lld",
                      n - (long long)outstart < 0 ? 0LL
                                                  : n - (long long)outstart);
  else if (i > 0) {
    long long est = ((long long)outend * (long long)r1->num) / i;
    job_result_printf(wi, "OUTOF %lld", est < 0 ? 0LL : est);
  }
}
static void intersect_emit_item(WorkItem *wi, int outstart, int outsize,
                                resultList *r1, resultList *r2) {
  Accum acc;
  long long n = 0;
  int outend = outstart + outsize;
  ...
  for (int i = 0; i < r1->num; ++i) {
    ...
    if (m != 0) {
      ...
      if (n <= outstart)
        continue;
      job_result_queue(wi, acc, r1->buf[i] + ((m - 1) << depth_shift),
                       r2->tags == NULL ? goodImages->tags[r] : r2->tags[r]);
      if (n >= outend)
        break;
    }
  ...
  job_result_flush(wi, acc);
  long long rem = n - (long long)outstart;
  job_result_printf(wi, "OUTOF %lld", rem < 0 ? 0LL : rem);
}
static void findFQV_emit_item(WorkItem *wi, int outstart, int outsize,
                              resultList *r1) {
  if (r1->num == 0) {
    job_result_printf(wi, "OUTOF %d", 0);
    return;
  ...
  if (k == 5) {
    long long rem = n - (long long)outstart;
    job_result_printf(wi, "OUTOF %lld", rem < 0 ? 0LL : rem);
  } else if (((long long)(k - 1) * (long long)r1->num + i) > 0) {
    long long est = ((long long)outend * (long long)r1->num * 3) /
                    ((long long)(k - 1) * (long long)r1->num + i);
    job_result_printf(wi, "OUTOF %lld", est < 0 ? 0LL : est);
  }
}
...
static void compute_thread_fn() {
  for (;;) {
    ...
    wi->status = JS_PREPROCESS;
    job_result_start(wi);
    // validate input
    if (wi->q.c1 < 0 || wi->q.c1 >= maxcat) {
      job_result_printf(wi, "ERROR C1RANGE %lld maxcat %d", (long long)wi->q.c1,
                        maxcat);
      job_result_printf(wi, "OUTOF %d", 0);
      job_result_done(wi);
      goto done;
    }
    {
      ...
      if (isFile((int)wi->q.c1)) {
        job_result_printf(wi, "ERROR C1ISFILE %lld cat=%d", (long long)wi->q.c1,
                          (int)cat[wi->q.c1]);
        job_result_printf(wi, "OUTOF %d", 0);
        job_result_done(wi);
        goto done;
    ...
    if (wi->q.a == std::string("path")) {
      result[0]->clear();
      wi->status = JS_STREAMING;
      tagCat_emit_item((tree_type)wi->q.c1,
                       (tree_type)(wi->q.c2 >= 0 ? wi->q.c2 : wi->q.c1),
                       (int)wi->q.d1, result[0], wi);
    } else {
      result[0]->num = 0;
      result[1]->num = 0;
      int cid[2] = {(int)wi->q.c1, (int)(wi->q.c2 >= 0 ? wi->q.c2 : wi->q.c1)};
      int depth[2] = {(int)wi->q.d1, (int)wi->q.d2};
      nr = (wi->q.a == "list" || wi->q.a == "fqv") ? 1 : 2;
      for (int j = 0; j < nr; ++j) {
        result[j]->clear();
        fetchFiles(cid[j], depth[j], result[j], &wi->cancel);
        if (wi->cancel.load())
          break;
      }
      ...
      if (!wi->cancel.load()) {
        if (wi->q.a == "list")
          traverse_emit_item(wi, wi->q.o, wi->q.s, result[0]);
        else if (wi->q.a == "fqv")
          findFQV_emit_item(wi, wi->q.o, wi->q.s, result[0]);
        else if (wi->q.a == "not")
          notin_emit_item(wi, wi->q.o, wi->q.s, result[0], result[1]);
        else
          intersect_emit_item(wi, wi->q.o, wi->q.s, result[0], result[1]);
      }
    }
    {
      time_t now = time(NULL);
      job_result_printf(wi, "DBAGE %.f", difftime(now, treetime));
      for (int j = 0; j < nr; ++j)
        result[j]->shrink();
    }
    wi->status = JS_DONE;
...
static int callback_http(struct lws *wsi, enum lws_callback_reasons reason,
  ...
  case LWS_CALLBACK_HTTP: {
    // If this is a WebSocket Upgrade request, do not send HTTP here.
#if defined(lws_http_is_upgrade)
    if (lws_http_is_upgrade(wsi))
      return 0;
#endif
    ...
    char uri[256];
    get_uri(wsi, uri, sizeof(uri));
    // WS alias path: never emit HTTP on it (lets upgrade proceed)
    ...
    if (strcmp(uri, "/status") == 0) {
      ...
      {
        std::lock_guard<std::mutex> lk(gq_mtx);
        qlen = (int)gqueue.size() + (g_current ? 1 : 0);
      }
      char body[256];
      snprintf(
          body, sizeof(body),
          "{\"queue\":%d,\"relsize\":%d,\"dbage\":%.f,\"load\":[%f,%f,%f]}",
          qlen, maxcat, difftime(now, treetime), la[0], la[1], la[2]);
      uint8_t buf[LWS_PRE + 512];
      uint8_t *p = &buf[LWS_PRE], *end = &buf[sizeof(buf) - 1];
      if (lws_add_http_common_headers(wsi, 200,
                                      "application/json; charset=utf-8",
                                      (lws_filepos_t)strlen(body), &p, end))
        return 1;
      if (lws_finalize_write_http_header(wsi, &buf[LWS_PRE], &p, end))
        return 1;
      lws_write(wsi, (unsigned char *)body, (unsigned int)strlen(body),
                LWS_WRITE_HTTP);
      return -1;
    }

    // Only accept compute on root path (except the WS path above)
    if (uri[0] != 0 && strcmp(uri, "/") != 0) {
      const char *body = "Bad request. Use /?c1=...&... or /status.\n";
      uint8_t buf[LWS_PRE + 512];
      uint8_t *p = &buf[LWS_PRE], *end = &buf[sizeof(buf) - 1];
      if (lws_add_http_common_headers(wsi, 400, "text/plain; charset=utf-8",
                                      (lws_filepos_t)strlen(body), &p, end))
        return 1;
      if (lws_finalize_write_http_header(wsi, &buf[LWS_PRE], &p, end))
        return 1;
    ...
    // initialize per-conn state (allocate under gq_mtx to synchronize with
    // worker)
    pss->wsi = wsi;
    pss->js = use_js;
    pss->closed = false;
    pss->js_header_sent = false;
    const char *ctype = use_js ? "application/javascript; charset=utf-8"
                               : "text/plain; charset=utf-8";
    uint8_t buf[LWS_PRE + 512];
    uint8_t *p = &buf[LWS_PRE], *end = &buf[sizeof(buf) - 1];
    if (lws_add_http_common_headers(wsi, 200, ctype, (lws_filepos_t)-1, &p,
                                    end))
      return 1;
    if (lws_finalize_write_http_header(wsi, &buf[LWS_PRE], &p, end))
      return 1;
    ...
    // enqueue work
    auto *wi = new WorkItem;
    wi->q = q;
    wi->ct = use_js ? CT_HTTP_JS : CT_HTTP;
    wi->ph = pss;
    pss->wi = wi;
    {
      std::lock_guard<std::mutex> lk(gq_mtx);
    ...
    if (use_js) {
      ...
      pss->q->push_back("fastcciCallback( [\n");
  ...
  case LWS_CALLBACK_HTTP_WRITEABLE: {
    ...
    if (pss->pend && pss->pend_off < pss->pend_len) {
      ...
      int n = lws_write(wsi, (unsigned char *)pss->pend + pss->pend_off,
                        (unsigned int)left, LWS_WRITE_HTTP);
      if (n < 0)
        return -1;
      pss->pend_off += (size_t)n;
      if (pss->pend_off < pss->pend_len) {
        if (pss->wsi)
          lws_callback_on_writable(wsi);
        return 0;
      }
      free(pss->pend);
      pss->pend = nullptr;
      pss->pend_len = pss->pend_off = 0;
      if (pss->pend_after_final) {
    ...
    if (!line.empty()) {
      ...
      pss->pend_len = out.size();
      pss->pend_off = 0;
      pss->pend_after_final = is_done;
  ...
  case LWS_CALLBACK_CLOSED_HTTP:
    ...
    if (pss->pend) {
      ...
      pss->pend = nullptr;
      pss->pend_len = pss->pend_off = 0;
      pss->pend_after_final = false;
    }
    if (pss->wi) {
      WorkItem *wi = pss->wi;
      wi->ph = nullptr;
      wi->cancel.store(true);
...
static int callback_ws(struct lws *wsi, enum lws_callback_reasons reason,
  ...
  case LWS_CALLBACK_FILTER_PROTOCOL_CONNECTION:
    // Accept all WS connections (no strict subprotocol check)
    return 0;

  case LWS_CALLBACK_ESTABLISHED: {
    char uri[256];
    get_uri(wsi, uri, sizeof(uri));
    TRACE_NOTICE("WS: ESTABLISHED uri='%s' proto='%s'", uri,
                 safe_proto_name(wsi));
    ...
    pss->wi = wi;
    {
      ...
      int pos = (int)gqueue.size() + (g_current ? 1 : 0);
      char msgbuf[64];
      snprintf(msgbuf, sizeof(msgbuf), "QUEUED %d\n", pos);
      emit_to_ws(pss, std::string(msgbuf));
      gqueue.push_back(wi);
    }
    gq_cv.notify_one();
    // Ensure we get a writeable to flush the first messages
    lws_callback_on_writable(wsi);
    return 0;
  }
  case LWS_CALLBACK_SERVER_WRITEABLE: {
    ...
    if (!pm) {
      // nothing to send; keep the connection open
      return 0;
    }
    std::vector<unsigned char> buf(LWS_PRE + pm->len);
    memcpy(&buf[LWS_PRE], pm->data, pm->len);
    int n =
        lws_write(wsi, &buf[LWS_PRE], (unsigned int)pm->len, LWS_WRITE_TEXT);
    if (n < 0)
      return -1;
...
int main(int argc, char **argv) {
  ...
  for (int i = 5; i > 0; --i) {
    result[0]->clear();
    result[0]->num = 0;
    fetchFiles(goodCats[i - 1][0], goodCats[i - 1][1], result[0], nullptr);
  ...
  if (pthread_create(
          ...
          [](void *) -> void * {
            ...
            for (;;) {
              ...
              {
                std::lock_guard<std::mutex> lk(gq_mtx);
                snapshot.assign(gqueue.begin(), gqueue.end());
                cur = g_current;
              }
              // send WAITING for queued WS items
              for (size_t i = 0; i < snapshot.size(); ++i) {
                WorkItem *wi = snapshot[i];
                if (wi && wi->ct == CT_WS && wi->pw) {
                  char msgbuf[64];
                  snprintf(msgbuf, sizeof(msgbuf), "WAITING %zu\n",
                           snapshot.size() - i);
                  emit_to_ws(wi->pw, std::string(msgbuf));
                }
              }
              // send WORKING for current
              if (cur) {
                if (cur->status == JS_PREPROCESS ||
                    cur->status == JS_COMPUTING) {
                  char msgbuf[64];
                  snprintf(msgbuf, sizeof(msgbuf), "WORKING %d %d\n",
                           result[0]->num, result[1]->num);
                  if (cur->ct == CT_WS && cur->pw)
                    emit_to_ws(cur->pw, std::string(msgbuf));
                  if ((cur->ct == CT_HTTP || cur->ct == CT_HTTP_JS) &&
                      cur->ph && !cur->ph->closed && cur->ph->q &&
                      cur->ph->qmtx) {
                    std::lock_guard<std::mutex> ql(*(cur->ph->qmtx));
                    cur->ph->q->push_back(std::string(msgbuf));
                    if (cur->ph->wsi)
                      lws_callback_on_writable(cur->ph->wsi);
                  }
                }
              }
            }
  ...
  lws_set_log_level(LLL_ERR | LLL_WARN | LLL_NOTICE | LLL_DEBUG, nullptr);
